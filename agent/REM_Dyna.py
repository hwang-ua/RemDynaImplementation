...
class REM_Dyna:
    ...
    def set_param(self, param):
        ...
        self.dim_ori_state = param["dim_state"]
        ...
        if DQN:
            ...
            if self.learning_mode == 0:
                if self.env[:7] == "Catcher":
                    node = [self.dim_state*self.binning, 100, self.num_action]
                else:
                    node = [2*self.binning, 512, self.num_action]
            elif self.learning_mode == 17:
                if self.env[:7] == "Catcher":
                    node = [param["nn_num_feature"]*self.binning, 100, self.num_action]
                else:
                    node = [32*self.binning, 512, self.num_action]
            else:
            ...
            self.dqn_learn_optimizer = torch.optim.Adam(self.dqn_learn.parameters(), lr = param["alpha"])
        ...
        if self.learning_mode == CHECK_DIST or \
            ...
            if self.graph:
                ...
                if self.env[:7] == "Catcher":
                    self.num_input = self.dim_ori_state#8192
                    self.num_output = self.num_input*2#8192* 2
                    self.num_node = param["nn_nodes"]
                    self.num_dec_node = param["nn_dec_nodes"]

                elif self.env == "ContinuousGridWorld":
                    self.num_input = 2
                    self.num_output = param["nn_num_tilings"] * param["nn_num_tiles"] * 2 * 2  # 4 #8192*2

                elif self.env == "PuddleWorld":
                    self.num_input = 2
                    self.num_output = param["nn_num_tilings"] * param["nn_num_tiles"] * 2 * 2 + 1  # 4 #8192*2

                if param["representation"] == "AE":
                    self.num_output = self.num_input
                    nn_post = "_AE"
                    rep_post = "_AE"
                elif param["representation"] == "noCons":
                    nn_post = ""
                    rep_post = "_noCons"
                elif param["representation"] == "new":
                    nn_post = ""
                    rep_post = "_new"
                elif param["representation"] == "newTC":
                    self.num_output -= 1
                    nn_post = ""
                    rep_post = "_newTC"
                elif param["representation"] == "newLegalv":
                    nn_post = ""
                    rep_post = "_newLegalv"

                self.num_node = param["nn_nodes" + nn_post]
                self.num_dec_node = param["nn_dec_nodes" + nn_post]
            ...
            self.rep_model = glr.GetLearnedRep(self.num_input,
                                               ...
                                               constraint=self.constraint,
                                               model_path=param["nn_model_path"],
                                               file_name=param["nn_model_name"+rep_post]
                                               )
            print("Loading representation:", param["nn_model_name"+rep_post], "\n")
        ...
        if self.offline and self.rem_type != "random_BufferOnly":
            ...
            if self.learning_mode in raw_model_mode_list:
                ...
                if LinearModel:
                    if self.env[:7] == "Catcher":
                        ...
                        fn = "prototypes/rem-GCov-100p-randomwalk/local_linear_model/mode0_trainingSetNormCov0.025_addProtLimit-0.025_baseOnState/model.pkl"
                    elif self.env == "PuddleWorld":
                        fn = "prototypes/rem-GCov-100p-randomwalk/local_linear_model/pw_mode0_trainingSetNormCov0.025_addProtLimit-0.06_kscale1.0_TCRscale40/model.pkl"
                    else:
                        print("Model error")
                        exit()
                    ...
                    # with open('prototypes/rem-GCov-100p-randomwalk/local_linear_model/pw_mode0_trainingSetNormCov0.025_addProtLimit-0.06_kscale1.0_TCRscale40/model.pkl', 'rb') as f:
                    ...
                    if self.env[:7] == "Catcher":
                        ...
                        fn = "prototypes/rem-GCov-100p-randomwalk/rem_model/mode0_onPolicyCov_addProtLimit-0.2_kscale1e-07/model.pkl"
                    elif self.env == "PuddleWorld":
                        fn = "prototypes/rem-GCov-100p-randomwalk/rem_model/pw_mode0_trainingSetNormCov0.0_addProtLimit-0.35_kscale1e-07_TCRscale40/model.pkl"
                    else:
                        print("Model error")
                        exit()
                    # with open('prototypes/rem-GCov-100p-randomwalk/rem_model/mode0_onPolicyCov_addProtLimit-0.2_kscale1e-07/model.pkl', 'rb') as f:
                    # with open('prototypes/rem-GCov-100p-randomwalk/rem_model/pw_mode0_trainingSetNormCov0.0_addProtLimit-0.35_kscale1e-07_TCRscale40/model.pkl', 'rb') as f:
                    with open(fn, 'rb') as f:
                        self.model = pkl.load(f)
            else:
                print("Loading rep model. LLM ==", LinearModel)
                print("Representation", param["representation"])
                print("Environment", self.env)
                if LinearModel:
                    if self.env[:7] == "Catcher":
                        fn = "prototypes/rem-GCov-500p-randomwalk/local_linear_model/catcher_mode10_trainingSetNormCov0.025_addProtLimit-22.0_kscale1.0_rep32/model.pkl"

                    elif self.env == "ContinuousGridWorld":
                        if param["representation"] == "new":
                            fn = "prototypes/rem-GCov-100p-randomwalk/local_linear_model/mode10_trainingSetNormCov0.025_addProtLimit-3.0_baseOnState/model.pkl"
                        elif param["representation"] == "newLegalv":
                            fn = "prototypes/rem-GCov-100p-randomwalk/local_linear_model/legalv_bw_mode10_trainingSetNormCov0.025_addProtLimit-1.5_kscale1.0/model.pkl"
                        elif param["representation"] == "AE":
                            fn = "prototypes/rem-GCov-100p-randomwalk/local_linear_model/AE_bw_mode10_trainingSetNormCov0.025_addProtLimit-0.05_baseOnState/model.pkl"
                        elif param["representation"] == "noCons":
                            fn = "prototypes/rem-GCov-100p-randomwalk/local_linear_model/noCons_bw_mode10_trainingSetNormCov0.025_addProtLimit-0.15_baseOnState/model.pkl"
                        else:
                            print("Model error")
                            exit()
                        ...
                        if param["representation"] == "new":
                            fn = "prototypes/rem-GCov-100p-randomwalk/local_linear_model/pw_mode10_trainingSetNormCov0.025_addProtLimit-2.25_kscale1.0_TCRscale40/model.pkl"
                        elif param["representation"] == "newTC":
                            fn = "prototypes/rem-GCov-100p-randomwalk/local_linear_model/pw_mode10_trainingSetNormCov0.025_addProtLimit-2.0_kscale1.0_TC/model.pkl"
                        elif param["representation"] == "AE":
                            fn = "prototypes/rem-GCov-100p-randomwalk/local_linear_model/AE_pw_mode10_trainingSetNormCov0.025_addProtLimit-0.025_kscale1.0/model.pkl"
                        elif param["representation"] == "noCons":
                            fn = "prototypes/rem-GCov-100p-randomwalk/local_linear_model/noCons_pw_mode10_trainingSetNormCov0.025_addProtLimit-0.3_kscale1.0_TCRscale40/model.pkl"
                        else:
                            print("Model error")
                            exit()
                    else:
                    ...
                    # with open('prototypes/rem-GCov-100p-randomwalk/local_linear_model/pw_mode10_trainingSetNormCov0.025_addProtLimit-2.25_kscale1.0_TCRscale40/model.pkl', 'rb') as f:
                    # with open('prototypes/rem-GCov-500p-randomwalk/local_linear_model/catcher_mode10_trainingSetNormCov0.025_addProtLimit-4.0_kscale1.0_rep8/model.pkl', 'rb') as f:
                    # with open('prototypes/rem-GCov-500p-randomwalk/local_linear_model/catcher_mode10_trainingSetNormCov0.025_addProtLimit-22.0_kscale1.0_rep32/model.pkl', 'rb') as f:
                    with open(fn, 'rb') as f:
                        self.model = pkl.load(f)
                else:
                    if self.env[:7] == "Catcher":
                        fn = "prototypes/rem-GCov-500p-randomwalk/rem_model/catcher_mode10_trainingSetNormCov0.0_addProtLimit-105.0_kscale1e-07_rep32/model.pkl"

                    elif self.env == "ContinuousGridWorld":
                        if param["representation"] == "new":
                            fn = "prototypes/rem-GCov-100p-randomwalk/rem_model/mode10_onPolicyCov_addProtLimit-65.0_kscale1e-07/model.pkl"
                        elif param["representation"] == "AE":
                            fn = "prototypes/rem-GCov-100p-randomwalk/rem_model/AE_bw_mode10_trainingSetNormCov0.0_addProtLimit-35.0_baseOnState/model.pkl"
                        elif param["representation"] == "noCons":
                            fn = "prototypes/rem-GCov-100p-randomwalk/rem_model/noCons_bw_mode10_trainingSetNormCov0.0_addProtLimit-45.0_baseOnState/model.pkl"
                        else:
                            print("Model error")
                            exit()
                        ...
                        if param["representation"] == "new":
                            fn = "prototypes/rem-GCov-100p-randomwalk/rem_model/pw_mode10_trainingSetNormCov0.0_addProtLimit-70.0_kscale1e-07_TCRscale40/model.pkl"
                        elif param["representation"] == "newTC":
                            fn = "prototypes/rem-GCov-100p-randomwalk/rem_model/pw_mode10_trainingSetNormCov0.0_addProtLimit-60.0_kscale1e-07_TC/model.pkl"
                        elif param["representation"] == "AE":
                            fn = "prototypes/rem-GCov-100p-randomwalk/rem_model/AE_pw_mode10_trainingSetNormCov0.0_addProtLimit-30.0_kscale1e-07_TCRscale40/model.pkl"
                        elif param["representation"] == "noCons":
                            fn = "prototypes/rem-GCov-100p-randomwalk/rem_model/noCons_pw_mode10_trainingSetNormCov0.0_addProtLimit-45.0_kscale1e-07_TCRscale40/model.pkl"
                        else:
                            print("Model error")
                            exit()
                    else:
                    ...
                    # with open('prototypes/rem-GCov-100p-randomwalk/rem_model/pw_mode10_trainingSetNormCov0.0_addProtLimit-70.0_kscale1e-07_TCRscale40/model.pkl', 'rb') as f:
                    # with open('prototypes/rem-GCov-500p-randomwalk/rem_model/catcher_mode10_trainingSetNormCov0.0_addProtLimit-15.0_kscale1e-07_rep8/model.pkl', 'rb') as f:
                    # with open('prototypes/rem-GCov-500p-randomwalk/rem_model/catcher_mode10_trainingSetNormCov0.0_addProtLimit-105.0_kscale1e-07_rep32/model.pkl', 'rb') as f:
                    with open(fn, 'rb') as f:
                        self.model = pkl.load(f)
                ...
                if DQN:
                    for _ in range(self.num_planning):#(1):
                        if self.rem_type == "random_BufferOnly":
                            ...
                            self.dqn_update_learn(seqs, True)
                        elif self.rem_type == "random" or \
                                self.rem_type == "random_pred" or\
                                self.rem_type == "pri_pred":
                            planned = 0
                            predictions = []
                            ...
                            self.dqn_update_learn(np.array(predictions), True)

                            if self.rem_type == "pri_pred" or \
                                    self.rem_type == "random_pred" or \
                                    self.rem_type == "pri":
                                preds = []
    ...
    def _feature_construction(self, state, action, isprint=False):
        if DQN:
            print("Using DQN. feature construction shouldn't be used")
            exit()
        #
        if self.learning_mode == SINGLE_REP or \
    ...
    # dqn
    def dqn_update_learn(self, seqs, do_planning):
        ...
        # if do_planning:
        #     loss = loss / np.sqrt(self.num_planning)
