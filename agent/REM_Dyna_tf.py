...
class REM_Dyna:
    ...
    def set_param(self, param):
        ...
        if DQN:
            ...
            self.dqn_learn = tf.keras.Sequential([tf.keras.layers.Dense(node[1], activation='selu', input_shape=(node[0],)),
                                                  tf.keras.layers.Dense(4)])
            self.dqn_target = tf.keras.Sequential([tf.keras.layers.Dense(node[1], activation='selu', input_shape=(node[0],)),
                                                   tf.keras.layers.Dense(4)])
            ...
            self.dqn_learn.compile(optimizer=tf.train.AdamOptimizer(param["alpha"]),
                                   loss='mse',
                                   metrics=['mae'])

            # self.dqn_loss = tf.losses.mean_squared_error
            # self.dqn_learn_optimizer = tf.train.AdamOptimizer(param["alpha"])
    ...
    def dqn_update_learn(self, seqs):
        ...
        s_values = self.dqn_learn.predict(s, batch_size = len(s))
        sp_values = self.dqn_target.predict(sp, batch_size = len(sp))
        prediction = s_values[:, a]
        sp_q_value = np.max(sp_values, axis=1)
        y = r + gamma * sp_q_value
        self.dqn_learn.fit()
        # loss = self.dqn_loss(prediction, y)
        # self.dqn_learn_optimizer.minimize(loss)
        # del s, a, s_values, prediction, gamma, r, sp, sp_values, sp_q_value, y, loss

    def synchronize_networks(self):
        del self.dqn_target
        self.dqn_target = tf.keras.models.clone_model(self.dqn_learn)
        self.dqn_target.set_weights(self.dqn_learn.get_weights())
