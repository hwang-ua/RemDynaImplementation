...
def RL_start():
    ...
    observation = {"state":last_state, "action":last_action}#, "plan_info": None}
...
def RL_step():
    ...
    if result['isTerminal'] == True:
        num_episodes += 1
        agent.agent_end(result['reward'], result['state'])
        result['action'] = None
    else:
        num_steps += 1
        last_action, other_info = agent.agent_step(result['reward'],result['state'])
        result['action'] = last_action
        result['other_info'] = other_info
    return result

def RL_step_debugging():
    global last_action, total_reward, num_steps, num_episodes
    result = environment.env_step(last_action)
    total_reward += result['reward']
    ...
    if result['isTerminal'] == True:
        ...
        num_steps += 1
        sample_sa = agent.agent_step_debugging(result['reward'], result['state'])
        real_seq = []
        for sa in sample_sa:
            # print(sa)
            new_x, new_y, reward, terminate = RL_env_message(["pseudo_model", sa])
            real_seq.append([[sa[0], sa[1]], sa[2], [new_x, new_y], reward, terminate])
        last_action = agent.agent_step_debugging_update_w(real_seq)
...
def RL_end():
    environment.env_end(None)
    #agent.agent_end(None)
