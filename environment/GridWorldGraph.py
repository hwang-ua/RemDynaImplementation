...
#
#         self.num_step = 0
#         return self._construct_graph([self.x, self.y])
#
#     def step(self, action):
#         x, y = self._next_position(action, self.x, self.y)
#
...
#         reward, terminate = self._check_goal()
#
#         return (self._construct_graph([self.x, self.y]), reward, terminate)
#
#     def _next_position(self, action, x, y):
#         if np.random.random() > self.suc_prob:
#             action = np.random.randint(0, 4)
...
#         graph = np.zeros((self.size_y * self.size_x), dtype=int)
#         graph[state[1] * self.size_x + state[0]] = 1
#         return graph
#
#
#     def numObservations(self):
