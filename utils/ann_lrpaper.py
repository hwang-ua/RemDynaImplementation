...
class NN():
    def __init__(self, num_input, num_node, num_output, learning_rate, weight_decay,
                 optimizer="adam", dropout = 0, momentum = 0, mode = "xy"):
    ...
    def training(self, s, sp, size_x, size_y, num_epochs, batch_size=1, legalv=0, print_loss = False):
        ...
        for epoch in range(num_epochs):
            ...
            while count < batch_size:
                ...
                s = dataset[s_idx]
                ft_s[count] = s[0]#len(prob_dist) - 1 - idx]
                ft_sp[count] = s[numpy.random.choice(len(prob_dist), p=prob_dist)]#-1]
                count += 1
            ...
            if legalv:
                ...
                while count < batch_size:
                    ...
                    s_idx_u = numpy.random.randint(len(dataset))
                    s_idx_v = numpy.random.randint(len(dataset))
                    u = dataset[s_idx_u]
                    v = dataset[s_idx_v]
                ...
                rd_u = numpy.random.random(size=len(ft_s) * 2).reshape((-1, 2)) * 2 - 1
