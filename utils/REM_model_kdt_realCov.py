...
from utils.get_learned_state import *
...
class REM_Model:
    def __init__(self, state_dim, num_near, add_prot_limit, model_params, learning_mode, similarity_limit, norm_diff,
        ...
        # if self.cov == 0:
        #     print("When sample mode = True, use training set cov")
        #     temp = np.load("random_data/fixed_env_suc_prob_1.0/cgw_noGoal_separateTC32x4_training_set_randomStart_0opt_[0.998, 0.8]gamma_1pts_x1_normCovM.npy")
        #     self.cov_inv = np.linalg.inv(temp + self.kscale)
        #     print(temp, self.cov_inv)
    ...
    def KDE_sampleSpRG(self, last_state, last_action):
        ...
        if sample is not None:
            ...
            # if np.max(state) > 1 or np.max(state) < 1:
    ...
    def sampleFromNext_pan(self, state, f, num_action):
        ...
        for p in predecessor_list:
            if p is not None:
                last_state, last_action, fake_state, reward, gamma = p
    ...
    def update_rem(self, last_state, last_action, state, reward, gamma):
        ...
        if last_action not in self.b_a:
            ...
            #REM update based on self.sample_mode
            rho, nnear_ind = self._sa_n_near(seq,sample_mode=self.sample_mode)
            rhor, nnear_ind2 = self._asp_n_near(seq,sample_mode=self.sample_mode)
            self.c[nnear_ind] = np.multiply((1.0 - rho), self.c[nnear_ind]) + \
                                np.multiply(rho, self._kernel_sprg(seq, knn_ind = nnear_ind,sample_mode=self.sample_mode)[0])
            self.cr[nnear_ind2] = np.multiply((1.0 - rhor), self.cr[nnear_ind2]) + \
                                 np.multiply(rhor, self._kernel_sa(seq, knn_ind = nnear_ind2,sample_mode=self.sample_mode)[0])
    ...
    def _kdtree_const(self, action):
        ...
        indices = [i for i in range(self.state_dim)]
        _, covmat_inv = self._cal_covmat_inv(indices, sample_mode="onPolicyCov")
    ...
    def _update_prototype(self, seq):
        ...
        if action not in self.same_a_ind.keys():
            ...
            _, covmat_inv = self._cal_covmat_inv(self.index_no_a, sample_mode="onPolicyCov")
            ...
            # # fixed covariance
            # _, covmat_inv = self._cal_covmat_inv(self.index_no_a, sample_mode="fixedCov")
    ...
    """
    def sample_sprg(self, last_state, last_action, depth=0, eval_mode=False):
        ...
        if self.b == 0:
            ...
            if self.fix_cov == 0:
                kernel, nz_ind = self._kernel_sa(seq, sample_mode="onPolicyCov")
            else:
                kernel, nz_ind = self._kernel_sa(seq, sample_mode="fixedCov")
            ...
            if eval_mode:
                ...
                if self.check_sampled:
                    candidates = self.prot_array[nz_ind, :self.state_dim]
    ...
    def _sample_predecessor(self, num_action, state, f):
        ...
        # sum_prob_sp = np.sum(self._kernel_sp(seq, sample_mode)[0])
    ...
    def _sample_s(self, last_action, state, seq, depth=0):
        ...
        if self.b == 0:
            ...
            if self.fix_cov == 0:
                kernel, nz_ind = self._kernel_asp(seq, sample_mode="onPolicyCov")
            else:
                kernel, nz_ind = self._kernel_asp(seq, sample_mode="fixedCov")
    ...
    """
    def _cal_covmat_inv(self, indices, sample_mode="fixedCov"):

        if sample_mode == "fixedCov":
            covmat_inv = np.eye(len(indices))*(1.0/self.cov)
            return None, covmat_inv
        elif sample_mode == "onPolicyCov":
