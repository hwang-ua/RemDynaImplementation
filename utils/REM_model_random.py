...
class REM_Model:
    ...
    def _set_random_proto(self):
        ind = np.array([int(i) for i in range(self.b)])
        np.random.shuffle(ind)
        self.random_ind = ind[:min(self.b, self.num_rand_proto)].astype(int)
        self.random_prot = np.copy(self.prot_array[self.random_ind])

    # Algorithm 7
    def update_rem(self, last_state, last_action, state, reward, gamma):
        self._set_random_proto()
        ...
        sa_n_near, rho, nnear_ind = self._sa_n_near(seq, self.random_prot)
        if self.b > 1:
            self.c[self.random_ind][nnear_ind] = np.multiply((1.0 - rho), self.c[self.random_ind][nnear_ind]) + \
                                             np.multiply(rho, self._kernel_sprg(seq, sa_n_near))

            asp_n_near, _, nnear_ind = self._asp_n_near(seq, self.random_prot)
            rhor = self._kernel_asp(seq, asp_n_near)
            self.cr[self.random_ind][nnear_ind] = np.multiply(1.0 - rhor, self.cr[self.random_ind][nnear_ind]) + \
    ...
    def _update_prototype(self, seq, num_near):
        ...
        if self.b == 0:
            ...
            dist_array = 1.0 - self._kernel_seq(seq, self.random_prot)
    ...
    def sample_sprg(self, last_state, last_action):
        if self.b == 0:
            ...
            betas = np.multiply(self.c[self.random_ind], self._kernel_sa(seq, self.random_prot))
            n_sa = np.sum(betas)
            if n_sa != 0 and not np.isnan(n_sa):
                betas /= float(n_sa)
                mu = betas.dot(self.random_prot[:, self.state_dim + 1: -1])
                diff = self.random_prot[:, self.state_dim + 1: -1] - mu

                cov = (diff * betas[:, None]).T.dot(diff)

                s_ind = np.random.choice(range(len(self.random_prot)), 1, p=betas)[0]
                target_mu = self.random_prot[s_ind, self.state_dim + 1: -1]
    ...
    def _sample_s(self, last_action, state):
        if self.b == 0:
            ...
            betas = np.multiply(self.cr[self.random_ind], self._kernel_asp(seq, self.random_prot))
            n_spa = np.sum(betas)
            ...
            mu = betas.dot(self.random_prot[:, :self.state_dim])
            shape = self.state_dim  # s
            cov = np.zeros((shape, shape))
            not_zero = np.where(betas != 0)[0]
            diff = self.random_prot[:, :self.state_dim] - mu

            cov = (diff * betas[:, None]).T.dot(diff) / float(self.b)  # n_spa

            s_ind = np.random.choice(range(len(self.random_prot)), 1, p=betas)[0]
            target_mu = self.random_prot[s_ind, :self.state_dim]
    ...
    def _prob_a_sp(self, last_action, state):
        occupied = [i for i in range(self.action_ind, self.state_dim * 2 + 1)]
        seq = self._refill_seq(np.concatenate((np.array([last_action]), state)), occupied)
        sum_prob_asp = np.sum(self._kernel_asp(seq, self.random_prot))
        sum_prob_sp = np.sum(self._kernel_sp(seq, self.random_prot))
        if sum_prob_sp == 0:
            return 1
        else:
