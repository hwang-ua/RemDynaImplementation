...
class REM_Model:
    ...
    def update_rem(self, last_state, last_action, state, reward, gamma):
        random_prot = np.array([i for i in range(min(self.b, self.num_rand_proto))])
    ...
    #         occupied = [i for i in range(self.state_dim + 1)]
    #         seq = self._refill_seq(np.concatenate((last_state, np.array([last_action]))), occupied)
    #         betas = np.multiply(self.c[:self.b], self._kernel_sa(seq, self.prot_array[:self.b]))
    #         n_sa = np.sum(betas)
    #         if n_sa != 0 and not np.isnan(n_sa):
    #             betas /= float(n_sa)
    #             mu = betas.dot(self.prot_array[:self.b, self.state_dim + 1: -1])
    #             diff = self.prot_array[:self.b, self.state_dim + 1: -1] - mu
    #
    #             cov = (diff * betas[:, None]).T.dot(diff)
    #
    ...
    def _sample_s(self, last_action, state):
        if self.b == 0:
            ...
            sampled_s = np.clip(np.random.multivariate_normal(target_mu, cov + np.eye(cov.shape[0]) * self.kscale), 0.,
                                1.)
