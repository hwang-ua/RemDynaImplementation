...
class REM_Model:
    def __init__(self, state_dim, num_near, add_prot_limit, model_params):
        ...
        self.c = np.ones((0))
        self.cr = np.ones((0))
        self.prot_array = np.empty((0, self.seq_dim))
    ...
    def update_rem(self, last_state, last_action, state, reward, gamma):
        ...
        sa_n_near, rho, nnear_ind = self._sa_n_near(seq, self.prot_array)
        self.c[nnear_ind] = np.multiply((1.0 - rho), self.c[nnear_ind]) + \
                            np.multiply(rho, self._kernel_sprg(seq, sa_n_near))

        asp_n_near, _, nnear_ind = self._asp_n_near(seq, self.prot_array)
    ...
    def _update_prototype(self, seq, num_near):
        ...
        if self.b == 0:
            ...
            dist_array = 1.0 - self._kernel_seq(seq, self.prot_array)
            nnear_ind = np.argsort(dist_array)
            add_prot = True if dist_array[nnear_ind[-1]] > self.add_prot_limit else False
        ...
        if add_prot:
            if self.b < self.prot_len:
                ...
                self.c = np.concatenate((self.c, np.array([1])), axis=0)
                self.cr = np.concatenate((self.cr, np.array([1])), axis=0)
            else:
                self.prot_array[self.b % self.prot_len] = seq
                self.c[self.b % self.prot_len] = 1
                self.cr[self.b % self.prot_len] = 1
            self.b += 1
    ...
    def sample_sprg(self, last_state, last_action):
        if self.b == 0:
            ...
            betas = np.multiply(self.c, self._kernel_sa(seq, self.prot_array))
            n_sa = np.sum(betas)
            if n_sa != 0 and not np.isnan(n_sa):
                betas /= float(n_sa)
                mu = betas.dot(self.prot_array[:, self.state_dim + 1: -1])
                diff = self.prot_array[:, self.state_dim + 1: -1] - mu

                cov = (diff * betas[:, None]).T.dot(diff)

                s_ind = np.random.choice(range(min(self.b, self.prot_len)), 1, p=betas)[0]
                target_mu = self.prot_array[s_ind, self.state_dim + 1: -1]
    ...
    def _sample_s(self, last_action, state):
        if self.b == 0:
            ...
            betas = np.multiply(self.cr, self._kernel_asp(seq, self.prot_array))
            n_spa = np.sum(betas)
            ...
            mu = betas.dot(self.prot_array[:, :self.state_dim])
            shape = self.state_dim  # s
            diff = self.prot_array[:, :self.state_dim] - mu

            cov = (diff * betas[:, None]).T.dot(diff) / float(min(self.b, self.prot_len))  # n_spa
    ...
    def _prob_a_sp(self, last_action, state):
        occupied = [i for i in range(self.action_ind, self.state_dim * 2 + 1)]
        seq = self._refill_seq(np.concatenate((np.array([last_action]), state)), occupied)
        sum_prob_asp = np.sum(self._kernel_asp(seq, self.prot_array))
        sum_prob_sp = np.sum(self._kernel_sp(seq, self.prot_array))
        if sum_prob_sp == 0:
            return 1
        else:
