...
class REM_Model:
    def __init__(self, state_dim, num_near, add_prot_limit, model_params):
        ...
        self.s_tree = [index.Index()] * self.num_action
        self.sp_tree = [index.Index()] * self.num_action
        self.sprg_tree = [index.Index()] * self.num_action
        self.ssprg_tree = [index.Index()] * self.num_action
        # self.s_tree = [kdt.KD_Tree(self.state_dim)] * self.num_action
        #         # self.sp_tree = [kdt.KD_Tree(self.state_dim)] * self.num_action
        #         # self.sprg_tree = [kdt.KD_Tree(self.state_dim + 2)] * self.num_action
        #         # self.ssprg_tree = [kdt.KD_Tree(self.seq_dim - 1)] * self.num_action
        return
    ...
    def _k_near_neighbor(self, tree, target, covmat_inv, ksize):
        # seqs, index, dist = tree.k_nearest(ksize, target, covmat_inv)
        hits = tree.nearest(target, ksize)
    ...
    def _update_prototype(self, seq, num_near):
        ...
        if self.b == 0:
            ...
            tree = self.ssprg_tree[int(seq[self.action_ind])]
            same_a = np.where(seq[self.action_ind] == self.prot_array[:, self.action_ind])[0]
            if same_a >= num_near:
        ...
        if add_prot:
            ...
            self.s_tree[int(seq[self.action_ind])].insert(self.b, seq[:self.state_dim], obj=self.b)
            self.sp_tree[int(seq[self.action_ind])].insert(self.b, seq[self.state_dim + 1:self.state_dim * 2 + 1], obj=self.b)
            self.sprg_tree[int(seq[self.action_ind])].insert(self.b, seq[self.state_dim + 1:], obj=self.b)
            self.ssprg_tree[int(seq[self.action_ind])].insert(self.b, seq[self.index_no_ag], obj=self.b)
            # self.s_tree[int(seq[self.action_ind])].insert(seq[:self.state_dim], self.b)
            # self.sp_tree[int(seq[self.action_ind])].insert(seq[self.state_dim + 1:self.state_dim * 2 + 1], self.b)
            # self.sprg_tree[int(seq[self.action_ind])].insert(seq[self.state_dim + 1:], self.b)
            # self.ssprg_tree[int(seq[self.action_ind])].insert(seq[self.index_no_ag], self.b)
    ...
    #         self.s_tree[i].rebuild()
    #         self.sp_tree[i].rebuild()
    #         self.sprg_tree[i].rebuild()
    #         self.ssprg_tree[i].rebuild()
    ...
    #         #                                     self.kernel_near)
    #         tree = self.ssprg_tree[int(seq[self.action_ind])]
    #         neighbor, index, dist = self._k_near_neighbor(tree, seq[0, indices], covmat_inv, self.kernel_near)
